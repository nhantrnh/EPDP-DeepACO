...
def test_main(args):
    ...
    print(f"Total execution time (Wall-clock): {wall_clock_duration:.2f}s")

    if all_results:
        total_solver_time = sum(r["time_seconds"] for r in all_results)
        avg_time_per_instance = total_solver_time / len(all_results)
        print(f"Average solve time per instance: {avg_time_per_instance:.3f}s")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Weight for the real cost in Neural LNS (w). 1.0 = classic LNS.",
    )
    parser.add_argument(
        "--num_workers",
        type=int,
