...
def get_aco_builder_args(pdp_info_dict):
    """
    Filters out valid parameters for ACO.__init__ from a large pdp_info dictionary.
    """
    valid_keys = [
        "node_types",
        "pair_ids",
...
def generate_on_the_fly_pdp_instance(
    ...
    """Helper function to generate data on-the-fly."""
...
def train_actor_critic_step(
    ...
    for _ in range(args.accumulation_steps):
        ...
        for i, instance_data in enumerate(instance_batch_list):
            ...
            mean_costs_ls.append(valid_ls_costs.mean().item())
...
def validation_pdp(
    ...
    """
    Runs parallel validation on the entire validation dataset.
    """
    model.eval()
...
def train_main(args):
    ...
    with open(config_filepath, "w") as f:
        ...
        print(f"Saved configuration to {config_filepath}")
    ...
    with open(log_filepath, "a", newline="") as log_file:
        ...
        for epoch in range(1, args.rl_epochs + 1):
            ...
            if val_cost < best_val_cost:
                ...
                torch.save(model.state_dict(), best_model_filepath)
                print(f"Saved new best RL model with Val Cost: {best_val_cost:.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Edge feature dimension, usually 1 (distance).",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="Low beta during training to encourage exploration.",
    )
    ...
    parser.add_argument(
        "--W_ls", type=float, default=1.0, help="Weight for the LS loss."
    )
    # ACO/LS Parameters for Validation
    parser.add_argument("--val_ants", type=int, default=50)
    parser.add_argument("--T_val_aco", type=int, default=100)
    parser.add_argument(
        "--val_aco_beta",
        type=float,
        default=5.0,
        help="High beta during validation to fully exploit the model.",
    )
    parser.add_argument(
        "--val_ls_mode",
        type=str,
